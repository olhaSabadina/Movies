...
/*
    func updateUser() {
        ...
        authService.$error
            .filter { $0 != nil }
            .sink { error in
                self.presentAlertWithTitle(title: "Error", message: error?.localizedDescription, buttonTitles: "OK", styleActionArray: [.default], alertStyle: .alert, completion: nil)
            }.store(in: &subscribers)
    }
    
    func fetchCurrentUser() {
        guard !authService.userIsAnonymously() else {
            self.user = UserProfile(name: "Anonymous", email: "Anonymous@mail.com")
    ...
    @objc private func deleteUser() {
        ...
        self.authService.deleteUser() { error in
            ...
            self.presentAlertWithTitle(title: error.localizedDescription, message: nil, buttonTitles: "", styleActionArray: [.default], alertStyle: .alert, completion: nil)
        }
    }
    ...
    private func editProfileAlert() {
        ...
        let okAction = UIAlertAction(title: "OK", style: .default) { _ in
            guard let name = alert.textFields?.first?.text,
                ...
                self.presentAlertWithTitle(title: "Error", message: "Fields must not be empty", buttonTitles: "OK", styleActionArray: [.default], alertStyle: .alert, completion: nil)
                return
            }
            user.name = name
            user.email = email
