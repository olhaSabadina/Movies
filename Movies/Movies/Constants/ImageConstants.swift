//
//  ImageConstants.swift
//  Movies
//
//  Created by Olga Sabadina on 05.01.2024.
//

import UIKit

enum ImageConstants {
    static let logoImage = UIImage(named: "logoImage")
    
    
    
    static let xmark = UIImage(systemName: "xmark.circle")
    static let plus = UIImage(systemName: "plus")
    static let menu = UIImage(named: "menu")
    static let calendar = UIImage(systemName: "calendar")
    static let qrcode = UIImage(systemName: "qrcode.viewfinder")
    static let firebase = UIImage(systemName: "icloud.and.arrow.down.fill")
    
    static let message = UIImage(systemName: "arrow.up.message")
    static let person = UIImage(systemName: "person.circle")
    static let logOut = UIImage(systemName: "figure.walk.departure")
    
    static let closeEye = UIImage(named: "closeEye")
    static let openEye = UIImage(systemName: "eye.fill")
    static let clear = UIImage(systemName: "multiply")
    static let emailOk = UIImage(systemName: "person.crop.circle.badge.checkmark")
    static let emailBusy = UIImage(systemName: "person.crop.circle.fill.badge.xmark")
}
